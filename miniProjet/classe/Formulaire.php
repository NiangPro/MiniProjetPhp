<?php 
	...
	class Formulaire
	{
		...
		protected function champsForm($name,$type = self::INPUT, $libelle = "", $valeur = ""){
			if ($type == self::PASSWORD || $type == self::INPUT || $type == self::EMAIL) {
				return '<div class="form-group">
			<lable>'.$libelle
			.'</label>
			<input type="'.$type.'" name="'.$name.'" value="'.$valeur.'" class="form-control" required>
			</div>';
			}elseif ($type == self::SUBMIT) {
				return '
				<button type="'.$type.'" class="btn btn-info" name="'.$name.'">'.$libelle.'</button>
				';
			}else{
				return '<a href="'.$type.'" class="btn btn-warning">'.$name.'</a>';
			}
		}
		public function input($name, $libelle, $valeur ="")
		{
			return $this->champsForm($name, self::INPUT, $libelle, $valeur);
		}
